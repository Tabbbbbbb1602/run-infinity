...
/// Trình quản lý trò chơi là một máy trạng thái, sẽ chuyển đổi giữa các trạng thái theo gamestate hiện tại
/// </summary>
public class GameManager : MonoBehaviour
{
    ...
    protected void OnEnable()
    {
        ...
        for(int i = 0; i < states.Length; ++i)
        {
            ...
            //Debug.Log(states[i].GetName());
        }

        m_StateStack.Clear();

        //Debug.Log(states[0].GetName());
        PushState(states[0].GetName());
    }

    //chuyển đổi các giữa 3 trạng thái
    protected void Update()
    {
        if(m_StateStack.Count > 0)
        {
            m_StateStack[m_StateStack.Count - 1].Tick();
            //Debug.Log(m_StateStack[m_StateStack.Count - 1]);
        }
    }
