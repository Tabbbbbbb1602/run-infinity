...
public class CharacterInputController : MonoBehaviour
{
	...
	protected void Update ()
    {
		...
		//difference input
		if(Input.GetKeyDown(KeyCode.A) && TutorialMoveCheck(0))
        {
			ChangeLane(-1);
        }
		else if (Input.GetKeyDown(KeyCode.D) && TutorialMoveCheck(0))
		{
			ChangeLane(1);
		}
		else if(Input.GetKeyDown(KeyCode.W) && TutorialMoveCheck(1))
        {
			Jump();
		}
		else if (Input.GetKeyDown(KeyCode.S) && TutorialMoveCheck(2))
		{
			if (!m_Sliding)
				Slide();
		}
		else if (Input.GetKeyDown(KeyCode.Space) && TutorialMoveCheck(1))
		{
			Jump();
		}
		...
		//Khi trượt xong quay lại trạng thái bình thường
		if (m_Sliding)
		{
			...
			if (ratio >= 1.0f)
			{
				...
				// Chúng tôi đã trượt đến (hoặc qua) độ dài cần thiết, quay lại chạy
				StopSliding();
			}
		}

		//Khi nhảy xong quay lại trạng thái bình thường
		if (m_Jumping)
        {
			if (trackManager.isMoving)
			{
				...
				// Tương tự như với trượt, chúng ta muốn một bước nhảy cố định CHIỀU DÀI không cố định THỜI GIAN bước nhảy. Ngoài ra, giống như trượt,
				// chúng tôi sửa đổi một chút độ dài theo tốc độ để làm cho nó dễ chơi hơn.
		...
		// Đặt bóng mờ dưới nhân vật.
		RaycastHit hit;
		//Debug.Log(Physics.Raycast(characterCollider.transform.position + Vector3.up, Vector3.down, out hit, k_ShadowRaycastDistance, m_ObstacleLayer));
	...
	public void ChangeLane(int direction)
    {
        ...
        if (targetLane < 0 || targetLane > 2)
			...
			// Bỏ qua, chúng ta đang ở trên biên giới.
    ...
    public void UseConsumable(Consumable c)
    {
        ...
        for(int i = 0; i < m_ActiveConsumables.Count; ++i)
        {
            if(m_ActiveConsumables[i].GetType() == c.GetType())
            {
				...
				// Nếu chúng ta đã có vật tư tiêu hao đang hoạt động thuộc loại đó, chúng ta chỉ cần đặt lại thời gian
        ...
        // Nếu chúng tôi chưa có, hãy kích hoạt cái đó
