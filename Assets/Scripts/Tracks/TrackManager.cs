...
public class TrackManager : MonoBehaviour
{
    ...
    public IEnumerator Begin()
    {
        if (!m_Rerun)
        {
            ...
            //Vì đây không phải là chạy lại, hãy khởi động toàn bộ hệ thống (khi chạy lại, chúng tôi muốn giữ các trạng thái mà chúng tôi đã có khi chết)
            ...
            //set các properti trên màn hình về thành 0 hết
    ...
    public void End()
    {
        ...
        //giải phóng đối tượng được tạo ra
        ...
        //nếu vật tư tiêu hao của chúng tôi không được sử dụng, chúng tôi đặt nó trở lại kho
    ...
    //Update: gọi mỗi frame một lần, nó thực sự phù hợp khi xử lý việc phản hồi liên tục
    void Update()
    {
        ...
        if (m_CurrentSegmentDistance > m_Segments[0].worldLength)
        {
            ...
            // m_PastSegment là phân đoạn chúng tôi đã vượt qua, chúng tôi giữ chúng để di chuyển chúng và hủy chúng sau
            //nhưng họ không còn là một phần của trò chơi nữa
        ...
        // Triển khai nguồn gốc nổi
        // Chuyển cả thế giới về 0,0,0 khi chúng ta đi quá xa.
        bool needRecenter = currentPos.sqrMagnitude > k_FloatingOriginThreshold;

        // Parallax Handling
        // Xử lý thị sai
        //Load lần lượt các lớp của cloud
        ...
        if (needRecenter)
        {
            ...
            // Tính toán lại vị trí thế giới hiện tại dựa trên thế giới đã di chuyển
            m_Segments[0].GetPointAtInWorldUnit(m_CurrentSegmentDistance, out currentPos, out currentRot);
        }
        ...
        if (parallaxRoot != null && currentTheme.cloudPrefabs.Length > 0)
        {
            for (int i = 0; i < parallaxRoot.childCount; ++i)
            {
                ...
                // Phá hủy những đám mây không cần thiết
        ...
        // Vẫn di chuyển đoạn trước cho đến khi chúng không còn hiển thị nữa.
        for (int i = 0; i < m_PastSegments.Count; ++i)
        {
        ...
        //nếu mà đã qua chạy thử
        if (!m_IsTutorial)
        {
            if (m_Speed < maxSpeed)
                //tăng tốc từ từ của m_Speed bằng k_Acceleration
        ...
        if (!m_IsTutorial)
        {
            ...
            //kiểm tra thứ hạng tiếp theo đạt được
...
//"level" in our game are milestone the player have to reach : one every 300m
    ...
    public IEnumerator SpawnCoinAndPowerup(TrackSegment segment)
    {
        if (!m_IsTutorial)
        {
            //Khoảng cách giữa các coin
            const float increment = 1.5f;

            //Khoảng cách xuất hiện giữa các lượt coin
            float currentWorldPos = 10.0f;
            int currentLane = Random.Range(0, 3);
            
            //Cơ hội xuất hiện powrup
            ...
            while (currentWorldPos < segment.worldLength)
            {
                ...
                //trả về true nếu có va chạm( tâm hình cầu( nhân vật, bán kính mặt cầu, các lớp Layer mask để bỏ qua các chọn lọc)
                ...
                if (laneValid)
                {
                    ...
                    //Random.value return 0.0 to 1.0
                    //powerupChance tỉ lệ xuất hiện powerUp
                    if (Random.value < powerupChance)
                    {
                        //Random.Range return min -> max ( min = 0, max = length of consumableDatabase.consumbales)
                        //pick powerUp in consumableDatabase.consumbales
                        ...
                        //nếu không thể tạo ra powerup, chúng tôi không đặt lại thời gian kể từ khi powerup tiếp tục để có cơ hội cao chọn một đoạn theo dõi tiếp theo
                        if (consumableDatabase.consumbales[picked].canBeSpawned)
                        {
                            // Spawn a powerup instead.
                            //Thay vào đó, sinh ra một sức mạnh.
                            // càng lên cao cơ hội sinh ra càng cao
                            m_TimeSincePowerup = 0.0f;
                            //Debug.Log(m_TimeSincePowerup);
                            powerupChance = 0.0f;

                            //
                            AsyncOperationHandle op = Addressables.InstantiateAsync(consumableDatabase.consumbales[picked].gameObject.name, pos, rot);
                            //Debug.Log(consumableDatabase.consumbales[picked].gameObject.name);
                            //Debug.Log(op.Result);
                            yield return op;
                            //
                            if (op.Result == null || !(op.Result is GameObject))
                    ...
                    else if (Random.value < premiumChance)
                    {
                       ...
                       /* Debug.Log(currentTheme.premiumCollectible.name);
                        Debug.Log(op);*/
                        yield return op;
                        if (op.Result == null || !(op.Result is GameObject))
                        {
